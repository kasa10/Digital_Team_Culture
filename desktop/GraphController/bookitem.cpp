...
BookItem::BookItem(Book *p, QGraphicsItem *parent)
    : G_OBJECT(parent)
{
    ...
    const double zoomAlpha=0.4; //Значение масштаба
    ...
    this->itemPix=new QGraphicsPixmapItem(QPixmap(":/assets/img/book.png"), this);
    itemPix->setScale(zoomAlpha);
    itemPix->setPos(-200, -60);

    this->group_id=new QGraphicsTextItem(this);
    this->group_id->setPlainText("Библиотечный номер: "+QString::number(this->book_obj->book_id));
    this->group_id->setPos(-200, 38);

    this->group_name = new QGraphicsTextItem(this);
    this->group_name->setPlainText(QString::fromStdString(this->book_obj->author).replace("\n"," ").left(30));
    this->group_name->setFont(font2);
    this->group_name->setPos(-90, -40);

    this->group_descr = new QGraphicsTextItem(this);
    this->group_descr->setPlainText(QString::fromStdString(this->book_obj->caption).replace("\t"," ").replace("\n"," ").left(200));
...
void BookItem::setPolygonSize(double width, double height)
{
    //  4          3
    //
    //
